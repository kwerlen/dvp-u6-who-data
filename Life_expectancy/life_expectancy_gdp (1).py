...
plt.show()
plt.savefig("country-LEABY_per_year.png")
...
print('life expectancy increased the most in Zimbabwe. ')
...
g3.set(ylabel = "Life expectancy at birth in years")
#plt.show()
...
#plt.show()
plt.savefig("GDPdevelopment.png")
...
print('highest GDP clearly in US. Zimbabwe and Mexicao on very low level, China starts from low level, but increases substantially.')
...
print('nevertheless, Zimbabwes life expectancy increased over the years, starting off from a very low level though')
...
g = sns.FacetGrid(df, col="Country", hue="Year", col_wrap=3, size=2)
g = (g.map(plt.scatter, "GDP", "LEABY", edgecolor="w").add_legend())
#plt.show()
